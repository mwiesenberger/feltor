...
\noindent
\begin{tcolorbox}[title=Note]
    Instead of temperature one can choose to initialise the pressure as well
    by writing `p` instead of `t` in the relevant field. However, only one of
    the two may be present not both.
\end{tcolorbox}
The "density", "tperp" and "tpara" fields all allow the following:
\begin{minted}[texcomments]{js}
"init":
{
    "density" : // or "tperp"/"pperp" or "tpara"/"ppara"
    {
...
% How to do it for source terms is also still open
