...
"boundary":
{
    "wall" :
    {
        ...
        "twall" : 0.1, // $T_w > 0$ wall temperature
        "qwall" : 0, // $Q_{w}$
    }
    "sheath":
    {
...
"boundary":
{
    ...
    "bc" :
    {
        "density" : [NEU, NEU], // boundary conditions in x and y for $N_s$
        ...
        // $U_{\parallel,s}$ for all $s$
        // DIR is in general not very stable, NEU works better
        "temperature" : [NEU, NEU], // boundary condition in x and y for
        // $T_\perp$ and $T_\parallel$ for all species (see \ref{sec:species})
        "tbc" : 0.1 // Dirichlet value; must be present if and only if any one
        // temperature boundary condition is DIR (see also $twall$ and background))
        "heat-flux" : [NEU, NEU], // boundary conditions in x and y for
        // heat flux $Q_\parallel$ and $Q_\perp$ for all $s$
        // DIR is in general not very stable, NEU works better
...
[ // list of dictionaries; its length determines the number of species
    {
        ...
        "mu" : 2.72121e-4, // mass relative to reference mass: $m_s/m_i$. $\mu<10^{-3}$
...
"init":
{
    ...
    // the file to use as a restart for $N_s$, $U_{\parallel,s}$, $T_{\parallel,s}$, $T_{\perp,s}$, $Q_{\parallel,s}$ and $Q_{\perp,s}$
}
\end{minted}
For this to work the species name in the previous simulation must match the current one.

In contrast, we can initialize the fields $N_s$, $U_{\parallel,s}$, $T_{\parallel,s}$, $T_{\perp,s}$, $Q_{\parallel,s}$ and $Q_{\perp,s}$
directly:
\begin{minted}[texcomments]{js}
"init":
{
...
\begin{tcolorbox}[title=Note]
    Instead of temperature one can choose to initializse the pressure as well
    by writing `p` instead of `t` in the relevant field. However, only one of
    the two may be present not both.
\end{tcolorbox}
The "density", "tperp" and "tparallel" fields all allow the following:
\begin{minted}[texcomments]{js}
"init":
{
    "density" : // or "tperp"/"pperp" or "tparallel"/"pparallel"
    {
...
"init":
{
    "velocity" :
    {
        ...
        // the "electrons" are the first massless species found
...
Per default we think of the initialisation of determining the physical, particle quantities however, the
...
% We should maybe start with 2 possibilities: "none" or "lwl"
% The question of how to do quasineutrality is somewhat related but also not
% and how to do it for source terms is also still open
...
At this stage we can have sources only in the density, perpendicular temperature and
parallel temperature equations. The parallel momentum and heat flux sources are always zero.
\begin{minted}[texcomments]{js}
"source":
{
    "density":
...
\noindent We can choose the source terms $S_N$, $S_{T,\perp}$ and $S_{T,\parallel}$ to force a profile
\begin{align} \label{eq:electron_source}
    S_{N,s}(R,Z,\varphi, t) &= \omega_s (n_{prof}(R,Z) - N_s(R,Z,\varphi, t))D(R,Z)
\end{align}
and analogously for the temperature / pressure sources,
...
"physical":
{
    ...
    "collisionality" : 1e-4, // parallel resistivity parameter Eq.\eqref{eq:resistivity}
...
The section `Steering the file output` in `feltor.pdf` applies verbatim.
